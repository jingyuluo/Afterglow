...
def CallBrilcalcForLumiAndProcess(onlineLumiDict,type="best"):
    ...
    if args.normtag:
        ...
        if type is "HFOC":
            lumiCMD.extend(["--normtag", "hfoc17v11"])#(["--type=hfoc", "--without-correction"])#(["--normtag", "hfoc16v4pre1"])
        if type is "HFET":
            lumiCMD.extend(["--normtag", "hfet17v11"])
        if type is "BCM1F":
            lumiCMD.extend(["--normtag", "bcm1f16v1"])
        if type is "PCC":
            lumiCMD.extend(["--normtag", "normtag_pcc_filtered.json"])#"pccLUM17001pre1"])#(["--type", "pxl"])#(["--normtag", "pccLUM17001pre6"])
                        ...
                        for i in range(len(bxlist)):
                            if i%3==0:
                                try:
                                    ...
                                    #print int(bxlist[i]), float(bxlist[i+1])
                                except: 
...
parser.add_argument('--nbxfrompcconly', action='store_true', default=False, help="Determine the number of bunch crossings from the PCC lumi.")
