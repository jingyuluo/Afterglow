...
parser.add_argument("--bunchtrain", action="store_true", default=False, help="Apply per-BX bunchtrain correction")
...
#templatefile=ROOT.TFile("/afs/cern.ch/work/j/jingyu/CMSSW_8_0_5/src/PCCTools/PCLumiProd/HFOC_template.root")
...
tree.SetBranchStatus("run*", 1)
tree.SetBranchStatus("LS*", 1)
tree.SetBranchStatus("HFOCLumi_perBX*", 1)
tree.SetBranchStatus("HFOCBXid*",1)
tree.SetBranchStatus("nBXHFOC*", 1)
...
for iev in range(nentries):
    ...
    #    if tree.LS>632:
    #        continue
    for ibx in range(tree.nBXHFOC):
        #print tree.HFOCLumi_perBX[ibx]
        if not args.all:
            allLumiPerBX[LBKey].Fill(tree.HFOCBXid[ibx], tree.HFOCLumi_perBX[ibx])
            normPerBX[LBKey].Fill(tree.HFOCBXid[ibx], 1)
        else:
            allLumiPerBX[LBKeys[0]].Fill(tree.HFOCBXid[ibx], tree.HFOCLumi_perBX[ibx])
            normPerBX[LBKeys[0]].Fill(tree.HFOCBXid[ibx], 1)
...
for LBKey in LBKeys:
    ...
    threshold = allCorrLumiPerBX[LBKey].GetMaximum()*0.1
    ...
    if args.bunchtrain:
        ...
        for ibx in range(2, allCorrLumiPerBX[LBKey].GetNbinsX()-5):
            ...
            if lumi>threshold and lumiM1> threshold:
                scale_fac = 1-0.00349*lumi+0.0001451*lumi*lumi
                
                allCorrLumiPerBX[LBKey].SetBinContent(ibx, lumi*scale_fac)
    ...
    for ibx in range(1, BXLength):
        if(allLumiPerBX[LBKey].GetBinContent(ibx)>allLumiPerBX[LBKey].GetMaximum()*0.1):
